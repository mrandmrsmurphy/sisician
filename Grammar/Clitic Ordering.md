...
Real Serbo-Croatian doesn't let clitics scatter freely — they cluster in a strict template, filling the sentence's "second position" (after the first stressed constituent). Adopted here wholesale, since there's no reason to stray from Croatian on this any more than on declension or the pronoun forms themselves:
...
## Why је Is Exiled to the End — a Real, Motivated Quirk

This isn't an arbitrary rule: **је** (3sg auxiliary "is") and **је** (accusative/genitive feminine clitic "her") are homophonous — both survive from [[Personal Pronouns]]'s own paradigm (3sg fem. genitive/accusative short form) and the "biti" auxiliary. Keeping them apart in the linear order (auxiliary је pushed to the very end, past the accusative/genitive slot where the other је would sit) is a real disambiguation strategy attested in actual Serbo-Croatian clitic syntax, not an invented complication. This is exactly the kind of "every weird feature should have a historical reason" result the project values — the ordering quirk *is* the explanation, not a decoration on top of one.

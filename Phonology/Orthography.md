# Orthography

Sisician's Cyrillic letter inventory. See [[../Culture/Sociolinguistic History|Culture/Sociolinguistic History]] § Orthographic History for *why* the language ends up written in Cyrillic despite its Latin-literate founding elite (short version: Cyrillic wins out as the normal literary script from c. 1400 onward; Latin survives only in diplomatic/foreign-trade/family-document contexts). This page is the letter-by-letter mapping itself. See [[../Sisician|Sisician]] for the hub page.

## Consonants (2026-09-19)

| | Labial | Alveolar | Palatal(ized) | Velar |
|---|---|---|---|---|
| Nasal | m м | n н | ɲ њ | (ŋ ӈ) |
| Stop (voiceless) | p п | t т | | k к |
| Stop (voiced) | b б | d д | | g г |
| Fricative (voiceless) | f ф | s с | š ш | x х |
| Fricative (voiced) | v в | z з | ž ж | |
| Affricate (voiceless) | | c ц | č ч | |
| Affricate (voiced) | | dz ѕ | dž џ | |
| Liquid | | l л | ʎ љ | |
| Liquid/Glide | | r р | j ј | |

Complete match against every consonant in [[Modern Inventory]] — nothing missing, nothing extra. **Note the "Palatal(ized)" column groups true palatals (ɲ ʎ j) with postalveolars (š ž č dž)** — a simplification for readability (Slavic phonological description often treats these together as the "soft" series), not a claim that š ž č dž are phonetically palatal rather than postalveolar; [[Modern Inventory]]'s own table keeps them in separate IPA columns.

**Real-world basis for each letter choice:**
- м н п т д к г ф в с з ш ж ц ч л р — standard Cyrillic, shared across Serbian/Russian/Macedonian.
- **љ, њ, ј, џ** — genuine single Cyrillic letters from Vuk Karadžić's Serbian Cyrillic reform (the "Vukovica"), not digraphs. љ/њ are historically ligature-derived from lj/nj; ј was adopted from the Latin alphabet specifically to distinguish /j/ from Russian's й.
- **ѕ** (for /dz/) — borrowed specifically from **Macedonian** Cyrillic, which has this letter; standard Serbian Cyrillic doesn't, because Serbian lacks phonemic /dz/. Sisician needs it because [[Modern Inventory]] has /dz/ as a real phoneme.
- **ӈ** (for the allophonic [ŋ]) — an extended Cyrillic letter from outside the South Slavic tradition (used in various Uralic/Siberian Cyrillic-based orthographies for /ŋ/). Shown in parentheses here because [ŋ] is only an allophone of /n/ before velars, not a separate phoneme — it may not need a dedicated letter at all in practice; flagged for a decision once actual spelling conventions get worked out.
- **No letters for ć/đ** (Serbian's ћ/ђ) — correctly absent, not an oversight: the vernacular merged these into č/j (see [[../Sources/Late Common Slavic|Sources/Late Common Slavic]] § Decided), so the phonemes don't exist to need letters. (They may still surface in the separate liturgical/OCS-derived written register — see [[../Sources/Old Church Slavonic|Sources/Old Church Slavonic]] § Consonants — which could need its own conventions later; not addressed here.)

## Vowels (confirmed 2026-09-19)

**/a e i o u/ → а е и о у** — plain standard Cyrillic vowel letters, no surprises, no separate yat-based spelling distinction (unlike Serbian's ije/je Ijekavian convention — Sisician's Ekavian-type yat reflex, per [[../Sources/Late Common Slavic|Sources/Late Common Slavic]] § Dialect Zone, doesn't need one).

## Open Items

- Whether [ŋ] gets a dedicated letter (ӈ) or is left unwritten/merged with н in practice.
- Any conventions for the liturgical/OCS-derived written register's ć/đ-type sounds, per [[../Culture/Sociolinguistic History|Culture/Sociolinguistic History]]'s note that the orthography shouldn't be perfectly transparent (etymological spellings, inherited conventions, old scribal habits are all in scope once this gets built out further).

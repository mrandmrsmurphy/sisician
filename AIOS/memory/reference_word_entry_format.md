...
**Demonstrative extension** (added 2026-09-23, first used by [[../../Words/тај|Words/тај]]): for `pos: pronoun` with `type: demonstrative`, drop `person` (demonstratives don't inflect for person) and add `demonstrative: proximal|medial|distal` (which deictic-distance slot — see [[../../Grammar/Demonstrative Pronouns|Grammar/Demonstrative Pronouns]] for why only proximal/medial are built so far), plus `gender: masculine|feminine` and `number: singular|plural` (demonstratives do inflect for both, unlike personal pronouns) and `case:` as usual. See [[../../Grammar/Demonstrative Pronouns|Grammar/Demonstrative Pronouns]] for the paradigm these pages plug into.

**Numeral extension** (added 2026-09-23, first used by [[../../Words/један|Words/један]]): for `pos: numeral`, add `value:` (the digit, 1–10 so far) and, only for один/два (the two numbers that inflect), `gender: masculine|feminine` — omit `gender` entirely for the invariant 3–10 words, per [[../../Grammar/Numerals|Grammar/Numerals]]'s own finding that real Croatian/Slovene agree numbers 3+ don't decline for gender. No `case`/`number`/`person` fields — numerals aren't full independent declensions any more than pronouns are.

**Verb extension** (added 2026-09-25, first used by [[../../Words/респонџати|Words/респонџати]], the vault's first verb `Words/` page): for `pos: verb`, drop `gender`/`declension` and add `conjugation: a-type|e-type|i-type` (per [[../../Grammar/Verb Paradigm (Sketch)|Grammar/Verb Paradigm (Sketch)]]'s three present-tense stem-vowel classes). Body's paradigm section gives the present-tense table (the only tense this vault has endings for so far, per that page's own "Explicitly Not Yet Built" list) rather than a declension table; link to [[../../Grammar/Verb Paradigm (Sketch)|Verb Paradigm (Sketch)]] for the class pattern instead of re-deriving it.

**Locative-adverb extension** (added 2026-09-25, first used by [[../../Words/овде|Words/овде]] and [[../../Words/тамо|Words/тамо]]): for `pos: adverb` with `type: demonstrative` (parallel to [[../../Words/кде|кде]]/[[../../Words/камо|камо]]'s existing `type: interrogative`), drop `gender`/`declension`/`person`/`case`/`number` entirely — these adverbs don't inflect at all — and add `demonstrative: proximal|medial` (which deictic-distance slot, mirroring the pronoun-side `demonstrative:` field on [[../../Words/овај|овај]]/[[../../Words/тај|тај]]). No paradigm/declension section in the body; these are single fossilized forms, the same shape as [[../../Words/кде|кде]]/[[../../Words/камо|камо]]'s own entries.

**Quantifier extension** (added 2026-09-25, first used by [[../../Words/все|Words/все]]): for `pos: pronoun` with `type: quantifier`, use `case:` as [[../../Words/кто|кто]] already does for a citation-form pronoun (e.g. `case: nominative, accusative` for a syncretic cell) but omit `gender`/`number`/`person` unless the specific word is a live agreeing cell of a fuller paradigm rather than a frozen/substantivized form — check the word's own Etymology section for whether that question is settled or explicitly left open before assuming either way.
